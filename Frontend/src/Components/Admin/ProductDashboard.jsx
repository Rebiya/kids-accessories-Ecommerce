...
// //               animate={{ scale: 1, y: 0 }}
// //               exit={{ scale: 0.9, y: 20 }}
// //               className="bg-black rounded-xl shadow-lg border border-gray-700 w-full max-w-md max-h-[90vh] overflow-y-auto"
// //             >
...
// //                         className="w-full p-2 bg-white border border-gray-600 rounded focus:outline-none focus:ring-2 focus:ring-yellow-500 text-white"
// //                         required
// //                       />
// //                     </div>
...
import 'react-toastify/dist/ReactToastify.css';
import { 
  FiEdit, FiTrash2, FiPlus, FiSearch, FiX, FiUpload, FiLogOut, 
  FiUsers, FiTag, FiShoppingBag, FiStar, FiDollarSign, 
  FiPieChart, FiShoppingCart 
} from 'react-icons/fi';
import { FaSpinner, FaCloudUploadAlt, FaChartLine } from 'react-icons/fa';
import { motion, AnimatePresence } from 'framer-motion';
...
const CategoryDashboard = () => (
  <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
    <h2 className="text-2xl font-bold text-gray-800 mb-4">Category Dashboard</h2>
    <p className="text-gray-600">This is the category management dashboard.</p>
  </div>
);

const OrderDashboard = () => (
  <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
    <h2 className="text-2xl font-bold text-gray-800 mb-4">Order Dashboard</h2>
    <p className="text-gray-600">This is the order management dashboard.</p>
  </div>
);
...
const UserDashboard = () => (
  <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
    ...
    <p className="text-gray-600">This is the user management dashboard.</p>
  </div>
);

const AnalyticalDashboard = () => (
  <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
    <h2 className="text-2xl font-bold text-gray-800 mb-4">Analytical Dashboard</h2>
    <p className="text-gray-600">This is the analytics dashboard with charts and metrics.</p>
  </div>
);
...
const ProductDashboard = () => {
  ...
  const fetchDataBasedOnTab = () => {
    switch (activeTab) {
      case 'products':
        fetchProducts();
        fetchCategories(); // Needed for product category dropdown
        break;
      case 'categories':
        fetchCategories();
        break;
      ...
      case 'users':
        ...
        break;
      case 'analytics':
        // No data fetching needed for analytics as it's a placeholder
        break;
      default:
        fetchProducts();
    }
  ...
  return (
    <div className="flex min-h-screen bg-gray-50 text-gray-800">
      ...
      <motion.div 
        ...
        animate={{ x: 0 }}
        transition={{ type: 'spring', stiffness: 300, damping: 30 }}
        className={`bg-white border-r border-gray-200 h-full fixed ${isSidebarCollapsed ? 'w-20' : 'w-64'} shadow-sm`}
      >
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 border-t border-gray-200">
          <button className="w-full flex items-center p-3 text-gray-700 hover:text-gray-900 rounded-lg transition-colors">
            <FiLogOut />
            {!isSidebarCollapsed && <span className="ml-3">Logout</span>}
          </button>
        </div>
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8">
            <div>
              <h1 className="text-2xl font-bold text-gray-800 mb-1">
                ...
                {activeTab === 'users' && 'User Management'}
                {activeTab === 'analytics' && 'Analytical Dashboard'}
              </h1>
              <p className="text-gray-600">
                ...
                {activeTab === 'users' && 'Manage system users'}
                {activeTab === 'analytics' && 'View system analytics and metrics'}
              </p>
            </div>
